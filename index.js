...
// const lockFilePath = path.join(__dirname, "./lock.json");
// let lockFileJson = {};
...
function filter() {
    ...
    console.log(
        chalk.green(`筛选图片完成，开始压缩图片，并发：${compressMaxLen}`)
    );
    ...
    function start() {
        compress().then((loadCount) => {
            ...
            if (current == total) {
                // fs.writeFileSync(
                //     lockFilePath,
                //     JSON.stringify(lockFileJson, null, 4)
                // );
    ...
    return new Promise((resolve) => {
        ...
        var req = https.request(options, function (res) {
            res.on("data", (buf) => {
                ...
                if (obj.error) {
                    console.log(
                        chalk.red(`[${img}]：压缩失败！报错：${obj.message}`)
                    );
                } else {
                ...
                fs.writeFile(imgpath, body, "binary", (err) => {
                    ...
                    console.log(
                        chalk.green(
                            `[${img}] 压缩成功，原始大小:${
                                obj.input.size
                            }kb，压缩大小:${obj.output.size}kb，优化比例:${
                                (1 - obj.output.ratio.toFixed(3)) * 100
                            }%`
                        )
                    );
                });
